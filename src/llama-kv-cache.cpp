...
static inline uint8_t turboquant_get_3bit(const uint8_t * src, size_t i) {
...
static inline void turboquant_set_3bit(uint8_t * dst, size_t i, uint8_t v) {
    const size_t bit = 3*i;
    const size_t byte = bit >> 3;
    const int shift = bit & 7;

    const uint16_t mask = uint16_t(0x7u) << shift;
    uint16_t word = dst[byte];
    if (shift > 5) {
        word |= uint16_t(dst[byte + 1]) << 8;
    }
    word = (word & ~mask) | ((uint16_t(v & 0x7u) << shift) & mask);

    dst[byte] = uint8_t(word & 0xffu);
    if (shift > 5) {
        dst[byte + 1] = uint8_t(word >> 8);
    }
}

static void turboquant_custom_pack_qjl_k(struct ggml_tensor * dst, int ith, int nth, void * userdata) {
    GGML_UNUSED(userdata);

    const ggml_tensor * k_unit        = dst->src[1];
    const ggml_tensor * k_idxs        = dst->src[2];
    const ggml_tensor * qjl_boundaries = dst->src[3];

    GGML_ASSERT(dst->type == GGML_TYPE_I8);
    GGML_ASSERT(k_unit->type == GGML_TYPE_F32);
    GGML_ASSERT(k_idxs->type == GGML_TYPE_I64);
    GGML_ASSERT(qjl_boundaries->type == GGML_TYPE_F32);

    const int64_t d_head   = k_unit->ne[0];
    const int64_t n_head   = k_unit->ne[1];
    const int64_t n_tokens = k_unit->ne[2];

    const int64_t packed_bytes_per_head = dst->ne[0] / n_head;
    const int64_t n_levels = qjl_boundaries->ne[1] + 1;

    GGML_ASSERT(packed_bytes_per_head == (int64_t) turboquant_pack_3bit_size((size_t) d_head));

    const int64_t dr = (n_tokens + nth - 1) / nth;
    const int64_t ir0 = dr * ith;
    const int64_t ir1 = std::min(ir0 + dr, n_tokens);

    const float * boundaries = (const float *) qjl_boundaries->data;

    for (int64_t t = ir0; t < ir1; ++t) {
        const int64_t cache_row = *(const int64_t *) ((const char *) k_idxs->data + t*k_idxs->nb[0]);
        uint8_t * dst_row = (uint8_t *) ((char *) dst->data + cache_row*dst->nb[1]);

        for (int64_t h = 0; h < n_head; ++h) {
            uint8_t * dst_head = dst_row + h*packed_bytes_per_head;
            memset(dst_head, 0, (size_t) packed_bytes_per_head);

            for (int64_t d = 0; d < d_head; ++d) {
                const float v = *(const float *) ((const char *) k_unit->data + d*k_unit->nb[0] + h*k_unit->nb[1] + t*k_unit->nb[2]);

                uint8_t level = 0;
                while (level + 1 < n_levels && v > boundaries[level]) {
                    ++level;
                }

                turboquant_set_3bit(dst_head, (size_t) d, level);
            }
        }
    }
}
...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            const size_t tensors_per_layer = qjl_enabled
                    ? size_t((qjl_corr_enabled ? 16u : 13u)*(1u + n_stream))
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        const uint32_t n_qjl_k_bytes_head = (uint32_t) turboquant_pack_3bit_size(hparams.n_embd_head_k);
        const uint32_t n_k_store_gqa = qjl_enabled ? n_qjl_k_bytes_head*hparams.n_head_kv(il) : n_embd_k_gqa;

        k       = ggml_new_tensor_3d(ctx, qjl_enabled ? GGML_TYPE_I8 : type_k, n_k_store_gqa, kv_size, n_stream);
        ...
        if (qjl_enabled) {
            ...
            if (qjl_corr_enabled) {
                ...
                k_esign = ggml_new_tensor_4d(ctx, GGML_TYPE_I8, qjl_m, hparams.n_head_kv(il), kv_size, n_stream);
            }
        ...
        for (uint32_t s = 0; s < n_stream; ++s) {
            k_stream.push_back(ggml_view_2d(ctx, k, n_k_store_gqa, kv_size, k->nb[1], s*k->nb[2]));
...

    {
        ...
        const char * k_type_name = layers.empty() ? ggml_type_name(type_k) : ggml_type_name(layers.front().k->type);

        LLAMA_LOG_INFO("%s: size = %7.2f MiB (%6u cells, %3d layers, %2u/%u seqs), K (%s): %7.2f MiB, V (%s): %7.2f MiB\n", __func__,
                (float)(memory_size_k + memory_size_v) / (1024.0f * 1024.0f), kv_size, (int) layers.size(), n_seq_max, n_stream,
                k_type_name, (float)memory_size_k / (1024.0f * 1024.0f),
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    const uint64_t n_head_kv    = hparams.n_head_kv(il);
    const uint64_t n_k_store_gqa = k->ne[0];

    const uint32_t ns = sinfo.s1 - sinfo.s0 + 1;

    if (qjl_enabled) {
        const uint64_t n_qjl_k_bytes_head = turboquant_pack_3bit_size(hparams.n_embd_head_k);
        GGML_ASSERT(n_k_store_gqa == n_qjl_k_bytes_head*n_head_kv);

        return ggml_view_4d(ctx, k,
                n_qjl_k_bytes_head, n_head_kv, n_kv, ns,
                ggml_row_size(k->type, n_qjl_k_bytes_head),
                ggml_row_size(k->type, n_k_store_gqa),
                ggml_row_size(k->type, n_k_store_gqa*kv_size),
                ggml_row_size(k->type, n_k_store_gqa*kv_size)*sinfo.s0);
    }

    GGML_ASSERT(n_k_store_gqa == n_embd_k_gqa);
...
ggml_tensor * llama_kv_cache::get_qjl_s(ggml_context * ctx, int32_t il) const {
    ...
    GGML_ASSERT(layers[ikv].qjl_s != nullptr);
    return ggml_view_tensor(ctx, layers[ikv].qjl_s);
}

ggml_tensor * llama_kv_cache::get_qjl_codebook(ggml_context * ctx, int32_t il) const {
    const int32_t ikv = map_layer_ids.at(il);

    return ggml_view_tensor(ctx, layers[ikv].qjl_codebook);
}
...
ggml_tensor * llama_kv_cache::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    const int64_t packed_bytes_per_head = (int64_t) turboquant_pack_3bit_size((size_t) n_embd_head);
    const int64_t packed_bytes_gqa = packed_bytes_per_head*n_head;
    ...
    const int64_t n_stream_enorm = k_enorm ? k_enorm->ne[3] : 0;
    const int64_t n_stream_esign = k_esign ? k_esign->ne[3] : 0;

    if (n_stream > 1) {
        const int64_t kv_size = get_size();

        assert(packed_bytes_gqa == k->ne[0]);
        assert(kv_size    == k->ne[1]);

        // merge the buffer across all streams because the idxs are global
        k = ggml_reshape_2d(ctx, k, packed_bytes_gqa, kv_size*n_stream);
    } else {
        k = ggml_reshape_2d(ctx, k, packed_bytes_gqa, get_size());
    }
    ...
    ggml_tensor * k_unit_heads = ggml_reshape_3d(ctx, k_unit, n_embd_head, n_head, n_tokens);
    ...
    if (!qjl_corr_enabled) {
        ...
        return ggml_custom_inplace(ctx, k, k_args, 4, turboquant_custom_pack_qjl_k, GGML_N_TASKS_MAX, nullptr);
    }
    ...
    ggml_tensor * k_hat_f32 = ggml_get_rows(ctx, layers[ikv].qjl_codebook, ggml_cont(ctx, idx_f32));
    ggml_tensor * e = ggml_sub(ctx, k_unit, k_hat_f32);
    ...
    return ggml_custom_inplace(ctx, k, k_args, 6, turboquant_custom_pack_qjl_k, GGML_N_TASKS_MAX, nullptr);
}
...
void llama_kv_cache::state_write_data(llama_io_write_i & io, const cell_ranges_t & cr) const {
    ...
    const uint32_t qjl_corr_state = qjl_corr_enabled ? 1 : 0;
    ...
    io.write(&qjl_corr_state, sizeof(qjl_corr_state));
    ...
    for (const auto & layer : layers) {
        ...
        if (!qjl_enabled) {
            ...
            const uint64_t k_size_row_packed = k->nb[1];
            io.write(&k_size_row_packed, sizeof(k_size_row_packed));

            for (const auto & range : cr.data) {
                const size_t range_size = range.second - range.first;
                const size_t buf_size = range_size * k_size_row_packed;
                io.write_tensor(k, range.first * k_size_row_packed, buf_size);
            }
            ...
            if (qjl_corr_enabled) {
                ...
                for (const auto & range : cr.data) {
                    ...
                    for (size_t row = 0; row < range_size; ++row) {
                        turboquant_pack_sign_1bit(k_esign_raw.data() + row*n_esign_el, n_esign_el, row_sign_packed);
...
bool llama_kv_cache::state_read_data(llama_io_read_i & io, uint32_t strm, uint32_t cell_count) {
    ...
    io.read_to(&qjl_corr_state, sizeof(qjl_corr_state));
    ...
    if (qjl_state != (uint32_t) (qjl_enabled ? 1 : 0) ||
        qjl_corr_state != (uint32_t) (qjl_corr_enabled ? 1 : 0) ||
        qjl_bits_state != qjl_bits || qjl_m_state != qjl_m) {
    ...
    for (const auto & layer : layers) {
        ...
        if (!qjl_enabled) {
            ...
            const size_t k_size_row_packed = k->nb[1];
            ...
            if (cell_count) {
                ggml_backend_tensor_set(k, io.read(cell_count * k_size_row_packed), head * k_size_row_packed, cell_count * k_size_row_packed);
            }
            ...
            if (qjl_corr_enabled) {
                ...
                if (cell_count) {
                    ...
                    ggml_backend_tensor_set(k_esign, raw_sign.data(), head * k_esign_size_row, cell_count * k_esign_size_row);
                }
            }
        }
    }
...
ggml_tensor * llama_kv_cache_context::get_qjl_codebook(ggml_context * ctx, int32_t il) const {
    return kv->get_qjl_codebook(ctx, il);
}
